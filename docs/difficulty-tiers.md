...
| **账户余额 / 积分 / 满减 / 会员** | 校园卡、积分、会员价 | ★★ | 已落地：`wallet`/`points`/`spend_discount`/`member_tier`；均开题扫描，须已有订单 |
| **售后/退款申请** | 申请退货退款 | ★★ | 已落地：**无独立 cap**；`refund_*` 列 + 用户申请 / 管理通过·驳回（回补库存+退账户余额） |
| **预约改约** | 改时段 | ★★ | 已落地：**无独立 cap**；`SlotStore.reschedule`（先释放原坑再占新坑；新坑失败则原约已取消） |
| **物流轨迹** | 快递跟踪 / 配送进度 | ★ | 已落地：**无独立 cap**；`OrderStore.logisticsTrace` + `OrderTraceDialog`（`el-timeline`，与审核进度同款）；按订单状态拼节点（含运输中/派送中）；不对接快递公司 API；自取为出餐→待取；收尾可为完成或售后 |
...
开题或客户点名下列能力时，**按加价项报价**，不写入本期交付承诺；基线用**系统内可答辩替代**（站内信、状态拼轨迹、账户余额等），或标「非本期」——不是空壳文案占位：

| 加价项 | 基线替代（可答辩） | 加价交付 |
|--------|-------------------|----------|
| 真短信 / 邮件通道 | 站内信 + 验证码（本地校验） | 运营商/SMTP |
...
| 真快递轨迹 API | 按订单状态拼物流节点（含运输中/派送中） | 快递公司回调/查询 |
| 微信/支付宝商户清算 | 账户余额扣/退 + 渠道与支付密码（不对接商户 SDK） | 商户进件与清算 |

### L2 · 交易 / 占坑积木（已落地，D/E 组可接）

| 项 | 运行时 | 参考域 |
|----|--------|--------|
| 购物车 / 多明细行订单 | `order_lines`：`OrderStore` + `/api/cart` + `/api/orders`（系统内余额/渠道密码；不对接商户 SDK） | SHOP / FOOD |
| 地址簿 / 履约字段 | `AddressStore` + 订单收货/配送/取餐码 | SHOP / FOOD |
| 售后 + 物流轨迹 | 订单 `refund_*`；`GET /api/orders/{id}/trace` + `OrderTraceDialog` | 凡 `order_lines` |
