...
|----|--------------|--------------|------------|------|
| M-01 | 选多家单位投递入职 | INTERN：默认「我的周报」选已建档岗；开题绑岗→资料绑岗 | 菜单/hero/种子；岗位说明≠我的多岗；≠ RECRUIT；`test_intern_bind` | 已齐 |
...
| M-24 | 食安风险排查 = 食堂点餐 | EVENT vs FOOD | 题名含食品安全/风险排查 → EVENT；正文食堂/档口勿抬点餐（`catalog` 硬分流 + `test_confusion_pairs` M-24） | 已齐 |

---

## 3. 深皮全册（落点=深皮，表内以已齐为主）

> 不新开 DOM；补 `keywords` / `scene_scan` / preset 文案 / 种子 / flavor / 样例开题，使该题型稳定命中挂靠域且界面不穿帮。新行默认 `待补`，收口后改 `已齐`。
...
| S-75 | 防返贫/帮扶户走访上报 | EVENT（household） | 户档 → 走访打卡 → 上报 | 词表+皮+种子；无资金直发 | 已齐 |
| S-76 | 裸献血管理（场次报名） | ACTIVITY（blood） | 场次 → 报名 → 审 | 补「献血管理」硬分流 | 已齐 |
| S-77 | 防暴恐/安全培训报名 | ACTIVITY（cert） | 项目 → 报名 → 审 | ± exam 开题点名 | 已齐 |
| S-78 | 歌剧院演出领票 | ACTIVITY（ticket） | 场次 → 领票 → 审 | ≠ CINEMA 选座 | 已齐 |
| S-79 | 学术期刊遴选荐购 | PROCURE（journal） | 品目 → 遴选单 → 审 | ≠ DOCLIB；无算法 | 已齐 |
| S-80 | 航班行李挂失认领 | LOST（baggage） | 启事 → 认领 → 审 | ≠ 全航迹/RFID | 已齐 |
| S-81 | 威客任务接单初筛 | RECRUIT（witkey） | 任务岗 → 投递 → 初筛 | ≠ 托管支付 | 已齐 |
...
| P-22 | 查寝 / 归寝签到（单据向） | DORM / ATTEND | **DOM-CHECKIN**：本人寝室绑定 + 归寝登记审核 + 口令签到（缺勤） | 资料绑寝 →「我的归寝」本寝登记 → 待审 → 签到中 → 口令已签到；窗后未签→缺勤 | 已齐 |
...
清单计数回写：2026-09-07（M=24 / S=60 / P=32）；此前：M-17～M-22、S-37/S-51 于 2026-08-12 追加；M-23 / P-32（CARRENT）已齐；S-75～S-81、M-24（食安≠点餐）于 2026-09 追加。组审计另计（见 §10）；P-01～P-08 于 2026-08-03 复审已齐；报修/工单、报名、交易、预约、内容组于 2026-08-12 收口。
