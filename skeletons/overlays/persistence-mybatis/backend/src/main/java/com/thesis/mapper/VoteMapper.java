...
public interface VoteMapper {
    ...
    @Select("SELECT id, campaign_id AS campaignId, name, intro, sort_no AS sortNo, status, avatar_url AS avatarUrl, created_at AS createdAt FROM vote_candidate WHERE campaign_id=#{campaignId} AND status='available' ORDER BY sort_no, id")
    List<Map<String, Object>> listCandidatesWithAvatar(long campaignId);
    ...
    @Select("SELECT id, campaign_id AS campaignId, name, intro, sort_no AS sortNo, status, avatar_url AS avatarUrl, created_at AS createdAt FROM vote_candidate WHERE campaign_id=#{campaignId} ORDER BY sort_no, id LIMIT #{limit} OFFSET #{offset}")
    List<Map<String, Object>> pageCandidatesWithAvatar(@Param("campaignId") long campaignId, @Param("limit") int limit, @Param("offset") int offset);
    ...
    @Insert("INSERT INTO vote_candidate(campaign_id, name, intro, sort_no, status, avatar_url) VALUES(#{campaignId}, #{name}, #{intro}, #{sortNo}, 'available', #{avatarUrl})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insertCandidateWithAvatar(Map<String, Object> row);
    ...
    @Select("SELECT c.id, c.name, c.intro, c.sort_no AS sortNo, c.avatar_url AS avatarUrl, COALESCE((SELECT COUNT(*) FROM vote_ballot b WHERE b.candidate_id=c.id),0) AS votes FROM vote_candidate c WHERE c.campaign_id=#{campaignId} ORDER BY votes DESC, c.sort_no, c.id")
    List<Map<String, Object>> resultsWithAvatar(long campaignId);
