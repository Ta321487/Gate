<template>
  <div>
    <p class="hint">预约时可以勾选这些要求，记在这一次入住上。</p>
    <div class="toolbar">
    ...
    <p v-if="!list.length" class="empty">还没有特殊要求。</p>
    <el-dialog v-model="visible" :title="form.id ? '编辑要求' : '添加要求'" width="420px">
...
async function load() {
  const res = await http.get('/api/boarding/cares/all')
  list.value = res.data || []
}
...
async function save() {
  ...
  await http.post('/api/boarding/cares', { ...form })
