    ...
    <el-table :data="list" stripe>
      ...
      <el-table-column label="操作" width="180" fixed="right">
        <template #default="{ row }">
          <el-button link type="warning" @click="remind(row)">催还</el-button>
          <el-button link type="primary" @click="ret(row)">归还</el-button>
        </template>
      </el-table-column>
...
async function ret(row) {
  await ElMessageBox.confirm(`确认归还？登记罚款 ${row.fineYuan || 0} 元。`, '归还')
