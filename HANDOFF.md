...
| `content` | ✅ | 公告 | 全部 |
| `guestbook` | ✅ | 门户留言（用户发 / 总管删与简短回复） | 默认 SHOP/FOOD/GENERIC·TRADE；开题写「留言」可扫入其它域（FORUM 除外） |
| `favorites` | ✅ | 商品收藏夹 | 默认 SHOP/FOOD；开题写「收藏」可扫入其它 order_lines 域 |
| `coupon` | ✅ | 优惠券完整生命周期 | 开题写「优惠券/领券」；领取→我的券→下单核销→过期扫标 |
| `order_review` | ✅ | 订单评价 | 开题写「订单评价/星级评价」；用户评 + 管理回复 |
| `search_assist` | ✅ | 标题前缀联想 + 配置热搜 | **仅开题写到才挂**（无域默认） |
| `browse_history` | ✅ | 最近浏览足迹 | **仅开题写到才挂** |
| `gallery` | ✅ | 档案多图（非 SKU） | **仅开题写到才挂** |
...
| `wallet` | ✅ | 演示余额扣/退（非真支付） | 开题写「余额/充值/校园卡」且已有订单 |
| `points` | ✅ | 消费积分入账（演示） | 开题写「积分」且已有订单 |
| `spend_discount` | ✅ | 满减算价 | 开题写「满减」且已有订单 |
| `member_tier` | ✅ | 会员等级折扣 | 开题写「会员等级/银卡金卡」且已有订单 |

依赖未实现能力 → `accept=reject`；上表均已落地，D/E 可 full。

**交易壳附带（无独立 cap，有 `order_lines` 即有）**：地址簿、售后申请/审、演示物流轨迹（`OrderTraceDialog`，与审核进度同款时间轴）。  
**超时关单（无独立 cap）**：开题写「超时取消/支付超时」→ `thesis.order-timeout-minutes=30` + `@Scheduled` 取消待确认单。  
**预约壳附带（有 `slot_reserve` 即有）**：管理端履约办结 `complete`、用户改约 `reschedule`。
...
| 领域 ID | 覆盖题目关键词 | 能力组合 |
|---------|----------------|----------|
| **DOM-SHOP** | 商城、二手、购物车、订单 | archive + order_lines + quota + content + org_users + **guestbook** + **favorites**（开题写券/余额/积分等可再扫 loyalty） |
| **DOM-FOOD** | 食堂、点餐、外卖档口 | 同上 |

交易答辩口径：下单 → 管理确认/发货 → 用户看物流轨迹 → 完成；可选券码、售后、收藏再加购。
...
**扫词 vs 附带**：收藏/券/忠诚度/订单评价靠开题词（或交易默认）进 caps；超时关单扫词写 yml 分钟数（无 cap）；搜索联想/浏览历史/多图**仅材料命中**才挂（无域默认）；售后/轨迹/改约随订单或预约壳自带，不单独扫词。
...
- 交易叠加收藏/足迹/券/评价时控制在 ≤15；论文 ER 按交付 SQL 如实画即可
...
- [x] L1 **`search_assist` / `browse_history` / `gallery`**：仅开题材料命中才挂；无词保持原样
