...
2. 仓库已经存在可用的完整索引，但当前Run没有可靠完整认知时，先调用 `aoci_rules`，再调用 `aoci_overview`。
   ...
   **传输失败与续传**：`next_cursor` 请求因 MCP 传输超时、连接中断或其他传输层故障失败时，属于传输故障而非语义错误，不得按 `cursor失败` 处理。允许对**同一** `next_cursor` 幂等重试至多 3 次（不得更换 cursor、不得改动 `chunk_tokens`、不得重新开始整链）。重试后仍失败时，保留已完成块的身份（`index_sha256`、各 `chunk_sha256`、`last_entry_ordinal`），提示用户二选一后继续：① 把该 MCP 服务的 `timeout` 提到 300 秒以上；② 把 `overview_delivery.chunk_tokens` 降到 4000 后重新开始。不得因传输故障作废整链，不得伪造 Attestation，也不得用 Memory／源码／`aoci.txt` 补答。
