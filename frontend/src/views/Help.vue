    ...
    cards: [
      {
        ...
        steps: [
          ...
          {
            title: '匹配确认',
            body: '在「匹配确认」页核验推荐骨架、领域与持久层等。确认无误后锁定匹配；如需调整，须先解锁再修改并重新确认。配色、布局等视觉选项在「一键生成」页调整。',
          },
          {
            title: '一键生成',
            body: '匹配确认后，在「一键生成」页调整视觉与生成选项（即时保存），再启动生成。启动前会弹出「开题对照」摘要，核对开题功能行是否落入工厂 checklist。生成完成后可同页改选项并「按当前选项重新生成」。进度可在「任务队列」与项目详情中查看；业务配置填充会拆成多个 Unit 并发执行，详情见「大模型与填岛拆解」。',
          },
          {
            title: '预览与验收',
      ...
      },
      {
        ...
        tag: '对照',
        wide: true,
        lead: '启动生成前弹出，将开题功能行与 Spec checklist 做静态对照（不调大模型），减少包后返工。',
        bullets: [
          '已对照：开题功能行与 checklist 条目一一命中。',
          '措辞弱匹配：字面不完全一致但语义相近，建议扫一眼。',
          '开题未落入 checklist：开题写了但工厂清单无对应项，须确认是否域选错或能力未挂载。',
          '对照结果不阻断生成，但强烈建议在匹配确认阶段已选对领域与骨架。',
        ],
      },
    ],
  },
    ...
    cards: [
      {
        ...
        bullets: [
          '实习管理系统：交周报（实习周报 INTERN）还是投简历找岗（招聘投递 RECRUIT）？INTERN≠同时入职多家；菜单是「岗位目录」，不是「我的=多单位」。',
    ...
    cards: [
      {
        ...
        terms: [
          ...
          { name: '匹配确认', def: '核验并锁定推荐骨架、领域、持久层与鉴权等。确认前不可正式生成；改动后须重新确认。视觉选项在「一键生成」页调整。' },
          ...
          { name: '开题对照', def: '生成前将开题功能行与 Spec checklist 静态比对；亦指质量检查中 checklist 核心项（P3a）。' },
