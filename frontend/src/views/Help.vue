...
const cards = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        title: '一键生成',
        body: '匹配确认后方可启动生成。生成以基线工程落地为主，进度可在「任务队列」与项目详情中查看。',
      },
      {
        title: '预览与验收',
        body: '于「运行」页启动前后端预览做快速验收；于「产物 / 对照」页核对质量检查、表结构、学生端 API 与 E-R 图。',
      },
      {
        title: '交付',
  ...
  },
  {
    title: '数据库结构',
    tag: '产物',
    lead: '生成完成后，可在「产物 / 对照」查看数据表；E-R 支持总图与分图，可复制图片或下载 PNG 贴进 Word。',
    bullets: [
  ...
  },
  {
    title: '学生端 API',
    tag: '产物',
    lead: '「产物 / 对照 → 学生端 API」自动扫描学生工程接口，列出方法、路径与处理函数，便于对照主流程验收。',
    bullets: [
      '按门户 / 管理端 / 基线等面筛选；可搜索路径或处理函数。',
      '命中主流程契约的接口会标注契约键（如申请 / 下单）。',
      '复制为「方法 + 路径」地址列表；联调基址取「运行」页后端地址。该视图仅供运营端验收，不含于学生交付包。',
    ],
  },
  {
    title: '工作台后端 API',
  ...
  },
  {
    ...
    bullets: [
      ...
      '「释放异常占用」只清理工作台未托管但仍占端口的进程；正在预览的项目须到项目详情停止。',
    ],
  },
  {
    title: '术语说明',
    ...
    terms: [
      ...
      { name: '生成配置', def: '描述业务配置的结构化数据（内部亦称 Spec / Schema）。大模型主要填充此类配置，而非手写业务代码。' },
      { name: '基线生成', def: '基于骨架与生成配置将工程落地的主路径（内部亦称 bake），区别于仅调用大模型写代码。' },
