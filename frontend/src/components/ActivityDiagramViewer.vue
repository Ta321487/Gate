<template>
  <div class="act-viewer" :class="{ 'is-dark': isDark }">
    <div class="act-toolbar row mb-12">
      <div class="act-zoom-btns row">
        <n-button size="small" @click="zoomOut">缩小</n-button>
        <span class="act-zoom-label">{{ Math.round(scale * 100) }}%</span>
    ...
    <p class="small muted act-hint mb-8">
      固定 3 张核心功能活动图：泳道为用户 / 系统 / 数据库；开始实心点、圆角任务、菱形判断（条件在菱形外侧，是/否在箭头上）、直角折线、结束外套圆。候选与序列图同源，文案取自 bake 包。
    </p>
    <div class="act-pick mb-8">
      <div class="small muted mb-6">从交付功能中勾选恰好 3 个（已选 {{ localIds.length }} / 3；默认与序列图同选）</div>
      <div class="act-cand-list">
        <label v-for="c in candidates" :key="c.id" class="act-cand">
          <input
            type="checkbox"
            :value="c.id"
          ...
          <span class="act-cand-meta muted">{{ c.actor }}</span>
        </label>
      </div>
      <n-button
        size="small"
    ...
    <div v-if="diagrams.length" class="act-tabs row mb-8">
      <n-button
    ...
    <p v-if="sourceNote" class="small muted act-hint mb-8">{{ sourceNote }}</p>
    <div
      ref="frameRef"
      class="act-frame"
    ...
    >
      ...
      <div v-else class="act-canvas" :style="canvasStyle" v-html="svgHtml" />
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  downloadName: { type: String, default: 'activity' },
...
const fileBase = computed(
  () => String(props.downloadName || 'activity').replace(/\.(svg|png)$/i, '') || 'activity',
)
...
function shortTitle(t) {
  const s = String(t || '').replace(/活动图$/u, '')
...
.act-frame.is-panning { cursor: grabbing; }
.act-viewer.is-dark .act-frame {
  background: #f7f7f7;
}
.act-canvas {
