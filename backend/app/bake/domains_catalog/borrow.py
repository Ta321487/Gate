...
DOMAINS: dict = {
    ...
    "DOM-EQUIP": {
        "label": "设备借用",
        # 词表预算 ≤20；戏服/户外拓展等长尾见 match_hint
        "keywords": [
            "设备借用", "器材", "实验室设备", "物资借用", "仪器借用",
            "实验器材", "设备租借",
            "雨伞租借", "充电宝租借", "门禁卡租借", "钥匙租借",
            "服装租借", "道具租借", "演出器材", "共享雨伞", "共享充电宝",
            "体育器材", "摄影器材", "乐器租借", "自行车租赁",
        ],
        "match_hint": (
            "适用：实验室/器材借用归还审核；亦覆盖校园轻资产、演出道具/戏服、体育/影像/乐器/教具/户外拓展、"
            "校园自行车/单车租赁或租借等换皮。"
            "勿与物资领用（耗材/试剂出库）、图书借阅、「仪器借用+机时预约」一体题或四轮商业租车（DOM-CARRENT）混淆"
            "（机时一体选 DOM-INSTRUMENT，勿只落本域而丢掉预约）；"
            "亦勿与哈啰式共享电单车扫码开锁物联网混淆（不在本期）。"
        ),
    ...
    "DOM-ASSET": {
        "label": "物资领用",
        # 词表预算 ≤20；冷库/货品台账等长尾见 match_hint
        "keywords": [
            "固定资产",
            "耗材申领",
            "物资申领",
        ...
        "match_hint": (
            "适用：物资/耗材/实训耗材/试剂/办公用品/劳保领用、仓储出入库、应急物资调度台账、冷链/冷库温湿度台账。"
    ...
    "DOM-EVENT": {
        ...
        "match_hint": (
            "适用：公卫应急、院感、晨检/晨午检、慢性病随访、养老/员工健康监测、网格排查、"
            "食安风险排查、流调协查、网格走访打卡、消防/设备巡检打卡、"
            "防返贫/帮扶对象建档走访上报、因病缺课监测等「建档+打卡/随访+上报」题。"
            "健康打卡/随访 ≠ 请销假（考勤请假）；请销假勿选本域。"
            "题名含食品安全/风险排查时选本域，勿因正文食堂/档口误落点餐。"
            "献血/开放日若主写「报名占名额」选活动报名，勿因出现献血一词误落本域。"
            "不适用：门诊挂号（医院）、食堂点餐（点餐）、纯应急物资仓储（物资领用）、"
            "宿舍卫生整改工单（宿舍报修）；亦勿承诺资金直发/大数据风控引擎。"
            "开题主写物资出入库勿选本域。"
    ...
    "DOM-RECRUIT": {
        ...
        "match_hint": (
            ...
            "勤工助学岗/助教岗申请、社会招聘/内部竞聘、威客/悬赏任务发布与接单投递初筛亦挂本域。"
            "威客题仅交「任务岗→投递→初筛」，勿承诺托管支付/担保交易/多级分润。"
    ...
    "DOM-DATING": {
        ...
        "match_hint": (
            ...
            "勿因题名仅出现「交友」就抢学习交友/学习搭子组队（组队匹配）；婚恋须有相亲/牵线/婚恋等词。"
    ...
    "DOM-INTERN": {
        ...
        "match_hint": (
            ...
            "主路径为在「我的周报」选已建档岗交周报，≠同时入职多家单位、≠投简历找岗。"
            "开题写岗位与学生绑定/一对一跟岗等则资料绑岗（个人资料单位+岗位对齐本岗）。"
