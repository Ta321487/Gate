...
PROFILE_FIELDS_BY_DOMAIN: dict[str, list[dict[str, Any]]] = {
    ...
    "DOM-SHOP": [
        ...
        _pf("campusAddress", "校内地址", required=True, on_register=True, max_length=128,
            required_when=_when("deliveryType", ["校内配送"]),
            visible_when=_when("deliveryType", ["校内配送"]),
            placeholder="宿舍楼栋 / 教学楼等"),
        _pf("dormOrBuilding", "楼栋宿舍", on_register=True, max_length=64,
            visible_when=_when("deliveryType", ["校内配送"])),
        _pf("offCampusAddress", "收货地址", required=True, on_register=True, max_length=128,
            required_when=_when("deliveryType", ["校外快递"]),
            visible_when=_when("deliveryType", ["校外快递"]),
            placeholder="校外详细地址"),
        _pf("defaultRemark", "下单备注偏好", max_length=128),
    ],
    "DOM-FOOD": [
        _pf("receiverName", "取餐人", required=True, on_register=True, max_length=32),
        _pf("pickupType", "取餐方式", required=True, on_register=True, field_type="select",
            options=["堂食/窗口", "宿舍配送", "其他"]),
        _pf("canteenPrefer", "常去食堂", on_register=True, max_length=64,
            visible_when=_when("pickupType", ["堂食/窗口"]),
            placeholder="选填"),
        _pf("campusAddress", "配送地址", on_register=True, max_length=128,
            required_when=_when("pickupType", ["宿舍配送"]),
            visible_when=_when("pickupType", ["宿舍配送"]),
            placeholder="宿舍楼栋与房间"),
