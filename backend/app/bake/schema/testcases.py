...
_BLUEPRINTS: dict[str, Blueprint] = {
    ...
    "blind_pools": [
        {
            "item": "维护盲盒奖池",
            "steps": ["进入「{label}」", "为盲盒填写奖品、权重和是否隐藏款", "保存"],
            "input": "奖品、权重、隐藏款",
            "expected": "奖池保存成功；买家购买的是盒子，开出奖品后才扣库存",
        }
    ],
    "group_campaigns": [
        {
            "item": "维护拼团",
            "steps": ["进入「{label}」", "填写成团人数和截止时间", "保存"],
            "input": "商品、成团人数、截止时间",
            "expected": "未截止的团可开团或参团；人齐后才能发货，过期未齐则取消并退回余额",
        }
    ],
    "delivery_slots": [
        {
            "item": "维护配送时段",
            "steps": ["进入「{label}」", "填写时段、余量和当日达或预订", "保存"],
            "input": "起止时刻、余量、送达方式",
            "expected": "结算时只能选择仍有余量的时段",
        }
    ],
    "price_spans": [
        {
            "item": "维护节日加价",
            "steps": ["进入「{label}」", "填写日期区间和倍率", "保存"],
            "input": "名称、起止日期、倍率",
            "expected": "下单当日命中区间时，加价计入本单",
        }
    ],
    "purchase_permits": [
        {
            "item": "审核购买申请",
            "steps": ["进入「{label}」", "打开待审申请", "通过或驳回"],
            "input": "通过，或驳回原因",
            "expected": "通过后才能加购和下单；驳回需填写原因",
        }
    ],
    "my_permits": [
        {
            "item": "提交购买审核",
            "steps": ["进入「{label}」", "上传资料并提交"],
            "input": "购买资料图片",
            "expected": "提交后为待审；通过前不能加购和下单",
        }
    ],
    "line_specs": [
        {
            "item": "维护规格选项",
            "steps": ["进入「{label}」", "新增或停用规格", "保存"],
            "input": "规格名称",
            "expected": "下单时从这份名单里选规格，刻字另记在订单上",
        }
    ],
    "lost_clues": [
        {
            "item": "查看线索留言",
            "steps": ["进入「{label}」", "查看留言"],
            "expected": "能看到路人线索；游客不必注册",
        }
    ],
    "consigns": [
        {
            "item": "质检寄卖",
            "steps": ["进入「{label}」", "打开待质检", "通过上架或驳回"],
            "input": "通过，或驳回原因",
            "expected": "通过后才上架；订单完成后按当时抽成记账",
        }
    ],
    "my_consigns": [
        {
            "item": "提交寄卖",
            "steps": ["进入「{label}」", "填写物品并提交"],
            "input": "名称、期望价、成色",
            "expected": "质检通过后才可购买；完成后可申请提现",
        }
    ],
    # 未知已交付菜单：仅冒烟，不发明业务规则
    "open": [
