...
class JobOut(BaseModel):
    ...
    kind: str = Field(default="bake", description="bake | defense_ppt")
    status: str = Field(description="状态")
    ...
    units: list[Any] = Field(default_factory=list, description="页级 Unit（PPT）")
...
class DeepSeekSettings(BaseModel):
    ...
    defense_ppt: bool = Field(default=True, description="答辩 PPT 填页")
    ...
    fill_unit_concurrency: int = Field(default=3, ge=1, le=8, description="填岛拆解并发 Unit 数")
...
class DeepSeekUpdate(BaseModel):
    ...
    defense_ppt: Optional[bool] = Field(default=None, description="答辩 PPT 填页")
    ...
    fill_unit_concurrency: Optional[int] = Field(default=None, ge=1, le=8, description="填岛拆解并发 Unit 数")
