...
SELECT '交易须知', '支持购物车、收货地址与多明细订单；下单后请选择收货地址或到店自提。', 'admin', '系统管理员'
